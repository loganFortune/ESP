...
def false_nearest_phase_space():
    ...
    f = open("temp_data_fnn_01_01.dat", "w")
    for i in range(0, len(data.sigbufs[channel])):
        f.write(str(data.sigbufs[channel][i]))
        f.write("\n")
    f.close()
    ...
    os.system('false_nearest temp_data_fnn_01_01.dat -x1000 -m3 -M1,15 -d39 -t1 -o output_data_chb01_01.fnn')

    f = open("output_data_chb01_01.fnn", "r")
    x = []
    y = []
    lecture = f.readlines()
    N_lignes = len(lecture)
    ...
    plt.plot(x, y)
    plt.savefig('false_nearest_chb01_01.png')
